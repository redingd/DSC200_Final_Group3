...
def api_function():
    ...
    # building the pandas df, setting the index to zip code, turning it into a csv file
    data_pd = pd.DataFrame(inner)
    data_pd.set_index("zip_code")
    data_pd.rename(columns={"zip_code": "Zip Code"}, inplace=True)
    data_pd.to_csv("./test_api_final.csv")
api_function()
...
def scrape_function():
    ...
    # making a csv file out of the dataframe
    df.to_csv("./test_scrape_final.csv")
scrape_function()
